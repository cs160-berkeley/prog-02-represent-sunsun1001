...
public class listReps extends AppCompatActivity {
    ...
    public static String[] bioID = {"F000062", "F000062", "F000062", "F000062"};
    public static String county = "Temp";
    public static String state = "Temp";
    public static String[] termDate = {"May 2016", "May 2016", "May 2016", "May 2016"};
    ...
    public class countyFind extends AsyncTask<Void, Void, Void> {
        ...
        /* Sunlight API Call */
            ...
            //Log.v("v", "mLat CountyFind: " + mLat);
            String mLong = getIntent().getStringExtra("mLong");
            String latZip = getIntent().getStringExtra("latZip");
            //Log.v("v", "latZip CountyFind: " + latZip);
            String longZip = getIntent().getStringExtra("longZip");
            try {

                /* Check if we need to query lat/long or zip */
                if (mLat != null && latZip.equalsIgnoreCase("0.0")) {

                    urlString.append("latlng=" + mLat+ "," + mLong);
                    urlString.append("&sensor=false");
                }

                else {
                    latZip = getIntent().getStringExtra("latZip");
                    longZip = getIntent().getStringExtra("longZip");
                    urlString.append("latlng=" + latZip + "," + longZip);
                    urlString.append("&sensor=false");
                }

                java.net.URL url = new URL(urlString.toString());
                Log.v("v", "URL CountyFind: " + url);
    ...
    public class SunlightData extends AsyncTask<Void, Void, Void> {
        ...
        /* Sunlight API Call */
            ...
            try {
             ...
             //   Log.v("v", "mLat Sunlight: " + mLat);
                ...
                /* Check if we need to query lat/long or zip */
                if (mLat != null && latZip.equalsIgnoreCase("0.0")) {
                    urlStringBuilder.append("&latitude=" + mLat + "&longitude=" + mLong);
                }
                else {
                    urlStringBuilder.append("&latitude=" + latZip + "&longitude=" + longZip);
                }

                String URL = urlStringBuilder.toString();
                Log.v("v", "URL Sunlight: " + url);
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            String elData = "";
            try {
                InputStream is = context.getAssets().open("election2012.json");
